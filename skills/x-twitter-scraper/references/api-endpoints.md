# Xquik REST API Endpoints

Routing index for the REST API reference. Load only the section file needed for the user task instead of loading the full endpoint catalog.

Base URL: `https://xquik.com/api/v1`

All requests require the `x-api-key` header unless the section says session auth only. All responses are JSON. HTTPS only.

Plan and credit changes are dashboard-only. Agents may read usage state with `GET /credits`, but must not start plan or credit changes.

| Need | Reference |
|---|---|
| Monitors | [monitors.md](api-endpoints-monitors.md) |
| Events | [events.md](api-endpoints-events.md) |
| Webhooks | [webhooks.md](api-endpoints-webhooks.md) |
| Draws | [draws.md](api-endpoints-draws.md) |
| Extractions | [extractions.md](api-endpoints-extractions.md) |
| X API (Direct Lookups) | [x-api.md](api-endpoints-x-api.md) |
| X Media (Download) | [x-media.md](api-endpoints-x-media.md) |
| Trends | [trends.md](api-endpoints-trends.md) |
| Radar | [radar.md](api-endpoints-radar.md) |
| Compose | [compose.md](api-endpoints-compose.md) |
| Drafts | [drafts.md](api-endpoints-drafts.md) |
| Tweet Style Cache | [tweet-style-cache.md](api-endpoints-tweet-style-cache.md) |
| X Accounts (Connected) | [x-accounts.md](api-endpoints-x-accounts.md) |
| X Write | [x-write.md](api-endpoints-x-write.md) |
| Credits | [credits.md](api-endpoints-credits.md) |
| Error Codes | [error-codes.md](api-endpoints-error-codes.md) |
