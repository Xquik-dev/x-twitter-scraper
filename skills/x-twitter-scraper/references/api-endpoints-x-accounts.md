...
Do not call these routes from this Skill. This list keeps the Skill docs aligned with the API and marks the dashboard-only boundary.
...
Show that full scope and list identities only after explicit confirmation.
...
Use method `DELETE` on route `/x/accounts/{id}`.
...
disconnecting it. Obtain explicit confirmation immediately before the call.
