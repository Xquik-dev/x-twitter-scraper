...
description: "Use when the user wants Twitter followers from a visible account. Filter profiles by verified status and export the results. Read-only."
license: MIT
metadata:
  internal: true
  author: Xquik
...
Export followers from any visible X account. Filter verified profiles or set a minimum follower count. Use an extraction above about 200 results.
...
3. Require user confirmation before running the metered extraction.
...
Extraction is metered. Show the estimate and ask for explicit confirmation before calling `POST /extractions`.
